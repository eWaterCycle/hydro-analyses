...
def dataframe_from_stationId(grdc_folder, station_id):
    ...
    grdc_data = grdc_data.rename(columns={'YYYY-MM-DD': 'date',
                                 ' Original': 'discharge'})
    grdc_data = grdc_data.reset_index().set_index(pd.DatetimeIndex(
                                                  grdc_data['date']))
...
def valid_series(cleaned, num_invalid_days=0):
    ...
    days of valid discharge data. num_invalid_days is the number of
    ...
    for column in original:
        ...
        for index, value in original[column].iteritems():
            if init:
                ...
                # in a NaN zone, looking for the next valid value so
