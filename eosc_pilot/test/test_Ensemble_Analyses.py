...
def test_set_2many_directories():
    with pytest.raises(Exception):
        EnsembleAnalyses('C:\forecast_dir', 'D:/grdc_dir', "E:/one2many")
...
def test_read_forecast():
    ...
    assert analysis.initialized is True
...
def test_read_grdc_with_forecast():
    ...
    discharge_value = data.sel(time="2018-06-14", lat=51.50, lon=0.0, method='nearest')["member13"]
    assert discharge_value >= 338.0
    assert discharge_value < 338.1
